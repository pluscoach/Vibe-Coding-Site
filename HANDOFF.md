...
## 11. 디자인 작업 검수 시스템 (강제 규칙, 2026-06-09 신설)

이미지 시안을 받아 디자인을 적용하는 모든 작업에 **강제 적용**. 예외 없음.

**절차:**
1. 시안 분석 — 글자 크기·간격·위치·톤·레이아웃 흐름을 항목별로 정리 (추측 금지)
2. 코드 수정 적용
3. 직접 캡쳐 — 헤드리스 브라우저(Edge/Chrome `--headless --screenshot`) 또는 Playwright. viewport는 시안 사이즈에 맞춤
4. 캡쳐를 직접 보고 시안과 비교 — 각 항목 차이 확인
5. 차이 있으면 재수정 → 다시 캡쳐 → 비교
6. **자동 재수정은 최대 2번까지**. 이후에도 시안과 다르면 즉시 사용자에게 확인 요청

**캡쳐 인프라 없으면 즉시 사용자에게 알린다. 캡쳐 없이 "완료" 보고 금지.**
